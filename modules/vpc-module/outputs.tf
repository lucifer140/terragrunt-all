...
output "private_subnets_1" {
  description = "List of only one subnet fo 1 avalabiltiy zone"
...
output "rt_ids" {
  value = concat(aws_route_table.public.*.id, aws_route_table.private.*.id, aws_default_vpc.this.*.default_route_table_id, [""])[0]
}
