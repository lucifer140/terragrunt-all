...
/*
resource "null_resource" "public_rt_id" {
  triggers = {
    dependency_id = var.public_rt_id
  }
 provisioner "local-exec" {
...
resource "null_resource" "private_rt_id" {
  triggers = {
    dependency_id = var.private_rt_id
  }
 provisioner "local-exec" {
    command = "python -c 'import time; time.sleep(90)'"
  }
...
resource "aws_route" "public-transit-gaetway" {
  for_each = var.tgw_route_object_public
...
resource "aws_route" "private-transit-gaetway" {
  for_each = var.tgw_route_object_private
  route_table_id         = each.value.route_table_id
  destination_cidr_block = each.value.destination_cidr_block
  gateway_id             = each.value.gateway_id
  //depends_on           = [null_resource.public_rt_id]
...
/*	
resource "aws_route" "public-transit-gaetway" {
  count = length(var.tgw-route-cidr-1)
  route_table_id         = aws_route_table.public[0].id
  destination_cidr_block = var.tgw-route-cidr-1[count.index]
  gateway_id             = var.transit_gateway_id
  //depends_on 		 = [null_resource.public_rt_id]
...
resource "aws_route" "public-transit-gaetway" {
  count = length(var.transit_route_cidr)
  route_table_id         = aws_route_table.public[0].id
  destination_cidr_block = var.transit_route_cidr[count.index]
  gateway_id             = var.transit_gateway_id
}
...
//  provider                        = aws.dst
  ...
  tags = {
    Name = "${element(var.public_name_tag, count.index)}-${element(var.azs, count.index)}"
    }
...
resource "aws_subnet" "private" {
 ...
 //depends_on                      = [var.transit_gateway_id]
  ...
  tags = {
    Name = "${element(var.private_name_tag, count.index)}-${element(var.azs, count.index)}"
        //element(var.azs, count.index),
    }
